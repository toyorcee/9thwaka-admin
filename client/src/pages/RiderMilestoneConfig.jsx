...
import { getAdminRiderMilestoneUsers } from '../services/adminApi';

const RiderMilestoneConfig = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await getAdminRiderMilestoneUsers({
        ...
        if (!response || response.success === false) {
          throw new Error(response?.error || 'Failed to fetch Rider Milestone users.');
        }
        ...
        setError(e.message || 'Failed to fetch Rider Milestone users.');
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow-md p-4 border-l-4 border-green-500">
          <p className="text-sm text-gray-500">Riders Achieving Milestones</p>
        ...
        <div className="bg-white rounded-lg shadow-md p-4 border-l-4 border-blue-500">
          <p className="text-sm text-gray-500">Total Milestone Funds Disbursed</p>
          <div className="flex justify-between items-end">
             <p className="text-2xl font-semibold text-gray-800">
        ...
        <div className="bg-white rounded-lg shadow-md p-4 border-l-4 border-yellow-500">
          <p className="text-sm text-gray-500">Active Config Tiers</p>
          <div className="flex justify-between items-end">
            <p className="text-2xl font-semibold text-gray-800">
              {stats.config?.tiers?.length ?? 0} Tiers
            </p>
    ...
    if (!items || items.length === 0) {
      return (
         <EmptyState
          ...
          description="Riders who progress towards or complete milestones will appear here."
        />
      );
    }
              ...
              {items.map((item) => (
                ...
                >
                  ...
                  <td className="py-3 px-4 whitespace-nowrap align-middle">
                    {item.nextTierTarget > 0 ? (
                      <>
                        <div className="w-full bg-gray-200 rounded-full h-2.5 max-w-[100px] mx-auto">
                           <div 
                               className={`bg-purple-600 h-2.5 rounded-full ${item.progress >= 100 ? 'bg-green-500' : 'bg-purple-500'}`} 
                               style={{ width: `${item.progress}%` }}
                           ></div>
                       </div>
                       <p className="text-xs text-center mt-1 text-gray-500">{item.progress}% to {item.nextTierTarget}</p>
                      </>
                    ) : (
                      <span className="text-xs text-gray-500">Wait for config</span>
                    )}
                  </td>
  ...
  return (
    <div className="p-6 h-full">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Rider Milestones Insights</h1>
        <p className="text-gray-600 mt-1">Track performing riders and the bonuses disbursed for their hustle.</p>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-4 rounded-lg shadow-sm mb-6 space-y-4 md:space-y-0 border border-gray-200">
        ...
        <div className="flex items-center space-x-4">
          ...
          >
            ...
            <option value="earned">Earned Payouts</option>
          </select>
        </div>
...
export default RiderMilestoneConfig;
